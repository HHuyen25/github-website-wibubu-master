...
if (isset($_POST['submit_review'])) {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        $review_message = '<div style="background: var(--gradient-glass); border: 1px solid rgba(239, 68, 68, 0.3); color: var(--color-error-700); padding: var(--space-lg) var(--space-xl); border-radius: var(--radius-lg); backdrop-filter: blur(10px); text-align: center; margin-bottom: var(--space-xl);">❌ Phiên làm việc không hợp lệ. Vui lòng thử lại!</div>';
    } else {
        $submitted_rating = (int)$_POST['rating'];
        $submitted_name = trim($_POST['reviewer_name']);
        $submitted_comment = trim($_POST['comment']);
        
        // Validate input
        if ($submitted_rating >= 1 && $submitted_rating <= 5 && 
            !empty($submitted_name) && !empty($submitted_comment) &&
            strlen($submitted_name) <= 100 && strlen($submitted_comment) <= 1000) {
        
        try {
            // Insert the review into database
            $stmt = $pdo->prepare("INSERT INTO reviews (product_id, reviewer_name, rating, comment) VALUES (?, ?, ?, ?)");
            $success = $stmt->execute([$product_id, $submitted_name, $submitted_rating, $submitted_comment]);
            
            if ($success) {
                $review_message = '<div style="background: var(--gradient-glass); border: 1px solid rgba(34, 197, 94, 0.3); color: var(--color-success-700); padding: var(--space-lg) var(--space-xl); border-radius: var(--radius-lg); backdrop-filter: blur(10px); text-align: center; margin-bottom: var(--space-xl);">✅ Cảm ơn bạn đã đánh giá! Đánh giá của bạn đã được gửi thành công.</div>';
                
                // Redirect to avoid resubmission
                header("Location: product-detail.php?id=" . $product_id . "&review_success=1");
                exit();
            } else {
                $review_message = '<div style="background: var(--gradient-glass); border: 1px solid rgba(239, 68, 68, 0.3); color: var(--color-error-700); padding: var(--space-lg) var(--space-xl); border-radius: var(--radius-lg); backdrop-filter: blur(10px); text-align: center; margin-bottom: var(--space-xl);">❌ Có lỗi xảy ra. Vui lòng thử lại!</div>';
            }
        } catch (Exception $e) {
            $review_message = '<div style="background: var(--gradient-glass); border: 1px solid rgba(239, 68, 68, 0.3); color: var(--color-error-700); padding: var(--space-lg) var(--space-xl); border-radius: var(--radius-lg); backdrop-filter: blur(10px); text-align: center; margin-bottom: var(--space-xl);">❌ Có lỗi xảy ra. Vui lòng thử lại!</div>';
        }
        } else {
            $review_message = '<div style="background: var(--gradient-glass); border: 1px solid rgba(239, 68, 68, 0.3); color: var(--color-error-700); padding: var(--space-lg) var(--space-xl); border-radius: var(--radius-lg); backdrop-filter: blur(10px); text-align: center; margin-bottom: var(--space-xl);">❌ Vui lòng điền đầy đủ thông tin hợp lệ!</div>';
        }
    }
}
    ...
    <div class="container">
        ...
        <main class="main-content">
            ...
            <section class="product-detail-container">
                ...
                <div style="margin-top: var(--space-6xl); animation: slide-in-up 0.9s ease-out;">
                    <!-- Rating Summary -->
                    <div style="background: var(--gradient-card); padding: var(--space-3xl); border-radius: var(--radius-xl); border: 1px solid rgba(255,255,255,0.3); backdrop-filter: blur(15px); box-shadow: var(--shadow-xl); margin-bottom: var(--space-3xl);">
                        <div style="text-align: center; margin-bottom: var(--space-2xl);">
                            <h3 style="font-size: var(--text-2xl); font-weight: 700; color: var(--color-neutral-900); margin-bottom: var(--space-lg);">⭐ Đánh Giá Sản Phẩm</h3>
                            
                            <?php if ($total_reviews > 0): ?>
                                <div style="display: flex; align-items: center; justify-content: center; gap: var(--space-lg); flex-wrap: wrap; margin-bottom: var(--space-xl);">
                                    <div style="display: flex; align-items: center; gap: var(--space-sm);">
                                        <span style="font-size: var(--text-4xl); font-weight: 700; color: var(--color-neutral-900);"><?php echo $average_rating; ?></span>
                                        <div style="display: flex; gap: var(--space-xs);">
                                            <?php for ($i = 1; $i <= 5; $i++): ?>
                                                <span style="font-size: var(--text-xl); color: <?php echo $i <= $average_rating ? '#FFD700' : '#E5E7EB'; ?>;">⭐</span>
                                            <?php endfor; ?>
                                        </div>
                                    </div>
                                    <div style="background: var(--gradient-glass); padding: var(--space-lg) var(--space-xl); border-radius: var(--radius-lg); border: 1px solid rgba(255,255,255,0.3); backdrop-filter: blur(10px);">
                                        <span style="font-size: var(--text-sm); color: var(--color-neutral-700);">Dựa trên </span>
                                        <span style="font-weight: 600; color: var(--color-neutral-800);"><?php echo $total_reviews; ?> đánh giá</span>
                                    </div>
                                </div>
                                ...
                                <div style="max-width: 400px; margin: 0 auto;">
                                    ...
                                    ?>
                                        <div style="display: flex; align-items: center; gap: var(--space-lg); margin-bottom: var(--space-sm);">
                                            <span style="font-size: var(--text-sm); color: var(--color-neutral-700); width: 60px;"><?php echo $star; ?> sao</span>
                                            <div style="flex: 1; background: var(--color-neutral-200); height: 8px; border-radius: var(--radius-full); overflow: hidden;">
                                                <div style="width: <?php echo $percentage; ?>%; height: 100%; background: var(--gradient-primary); border-radius: var(--radius-full);"></div>
                                            </div>
                                            <span style="font-size: var(--text-sm); color: var(--color-neutral-600); width: 30px;"><?php echo $count; ?></span>
                                        </div>
                                    <?php endfor; ?>
                                </div>
                            <?php else: ?>
                                <div style="text-align: center; padding: var(--space-2xl);">
                                    <div style="font-size: var(--text-6xl); margin-bottom: var(--space-lg); opacity: 0.5;">⭐</div>
                                    <p style="color: var(--color-neutral-600); font-size: var(--text-lg);">Chưa có đánh giá nào</p>
                                    <p style="color: var(--color-neutral-500); font-size: var(--text-sm); margin-top: var(--space-sm);">Hãy là người đầu tiên đánh giá sản phẩm này!</p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <?php if (!empty($reviews)): ?>
                        <div style="margin-bottom: var(--space-3xl);">
                            <h4 style="font-size: var(--text-xl); font-weight: 600; color: var(--color-neutral-900); margin-bottom: var(--space-2xl); text-align: center;">💬 Bình Luận Từ Khách Hàng</h4>
                            
                            <div style="display: grid; gap: var(--space-xl);">
                                <?php foreach ($reviews as $review): ?>
                                    <div style="background: var(--gradient-card); padding: var(--space-2xl); border-radius: var(--radius-xl); border: 1px solid rgba(255,255,255,0.3); backdrop-filter: blur(15px); box-shadow: var(--shadow-lg); transition: all var(--transition-fast);" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='var(--shadow-xl)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='var(--shadow-lg)'">
                                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: var(--space-lg); flex-wrap: wrap; gap: var(--space-sm);">
                                            <div style="display: flex; align-items: center; gap: var(--space-lg);">
                                                <div style="background: var(--gradient-glass); padding: var(--space-sm); border-radius: var(--radius-full); border: 1px solid rgba(255,255,255,0.3); backdrop-filter: blur(10px); display: flex; align-items: center; justify-content: center;">
                                                    <span style="font-size: var(--text-lg);">👤</span>
                                                </div>
                                                <div>
                                                    <div style="font-weight: 600; color: var(--color-neutral-800); margin-bottom: var(--space-xs);">
                                                        <?php echo htmlspecialchars($review['reviewer_name'] ?: 'Khách hàng'); ?>
                                                    </div>
                                                    <div style="display: flex; gap: var(--space-xs);">
                                                        <?php for ($i = 1; $i <= 5; $i++): ?>
                                                            <span style="font-size: var(--text-sm); color: <?php echo $i <= $review['rating'] ? '#FFD700' : '#E5E7EB'; ?>;">⭐</span>
                                                        <?php endfor; ?>
                                                    </div>
                                                </div>
                                            </div>
                                            <div style="font-size: var(--text-sm); color: var(--color-neutral-500); background: var(--gradient-glass); padding: var(--space-sm) var(--space-lg); border-radius: var(--radius-full); border: 1px solid rgba(255,255,255,0.3); backdrop-filter: blur(10px);">
                                                <?php echo date('d/m/Y', strtotime($review['created_at'])); ?>
                                            </div>
                                        </div>
                                        
                                        <?php if ($review['comment']): ?>
                                            <div style="background: var(--gradient-glass); padding: var(--space-xl); border-radius: var(--radius-lg); border: 1px solid rgba(255,255,255,0.3); backdrop-filter: blur(10px);">
                                                <p style="color: var(--color-neutral-700); line-height: 1.6; margin: 0;">
                    ...
                    <!-- Add Review Form -->
                    <div style="margin-top: var(--space-4xl); background: var(--gradient-card); padding: var(--space-3xl); border-radius: var(--radius-xl); border: 1px solid rgba(255,255,255,0.3); backdrop-filter: blur(15px); box-shadow: var(--shadow-xl);">
                        <div style="text-align: center; margin-bottom: var(--space-2xl);">
                            <h4 style="font-size: var(--text-xl); font-weight: 600; color: var(--color-neutral-900); margin-bottom: var(--space-lg);">✍️ Viết Đánh Giá Của Bạn</h4>
                            <p style="color: var(--color-neutral-600);">Chia sẻ trải nghiệm của bạn để giúp khách hàng khác</p>
                        </div>
                        ...
                        <form method="POST" action="product-detail.php?id=<?php echo $product_id; ?>" style="max-width: 600px; margin: 0 auto;">
                            <input type="hidden" name="product_id" value="<?php echo $product_id; ?>">
                            <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
                            
                            <!-- Star Rating Selection -->
                            <div style="margin-bottom: var(--space-2xl);">
                                <label style="display: block; font-weight: 600; color: var(--color-neutral-800); margin-bottom: var(--space-lg); text-align: center;">Đánh giá của bạn:</label>
                                <div style="display: flex; justify-content: center; gap: var(--space-sm); margin-bottom: var(--space-lg);">
                                    ...
                                    <label for="star1" class="star-rating" onclick="setRating(1)" style="font-size: var(--text-2xl); color: #E5E7EB; cursor: pointer; transition: color var(--transition-fast);" onmouseover="previewRating(1)" onmouseout="resetPreview()">⭐</label>
                                    
                                    <input type="radio" name="rating" value="2" id="star2" style="display: none;">
                                    <label for="star2" class="star-rating" onclick="setRating(2)" style="font-size: var(--text-2xl); color: #E5E7EB; cursor: pointer; transition: color var(--transition-fast);" onmouseover="previewRating(2)" onmouseout="resetPreview()">⭐</label>
                                    
                                    <input type="radio" name="rating" value="3" id="star3" style="display: none;">
                                    <label for="star3" class="star-rating" onclick="setRating(3)" style="font-size: var(--text-2xl); color: #E5E7EB; cursor: pointer; transition: color var(--transition-fast);" onmouseover="previewRating(3)" onmouseout="resetPreview()">⭐</label>
                                    
                                    <input type="radio" name="rating" value="4" id="star4" style="display: none;">
                                    <label for="star4" class="star-rating" onclick="setRating(4)" style="font-size: var(--text-2xl); color: #E5E7EB; cursor: pointer; transition: color var(--transition-fast);" onmouseover="previewRating(4)" onmouseout="resetPreview()">⭐</label>
                                    
                                    <input type="radio" name="rating" value="5" id="star5" style="display: none;">
                                    <label for="star5" class="star-rating" onclick="setRating(5)" style="font-size: var(--text-2xl); color: #E5E7EB; cursor: pointer; transition: color var(--transition-fast);" onmouseover="previewRating(5)" onmouseout="resetPreview()">⭐</label>
                                </div>
                                <div id="rating-text" style="text-align: center; font-size: var(--text-sm); color: var(--color-neutral-600); min-height: 20px;"></div>
                            </div>
                            
                            <!-- Reviewer Name -->
                            <div style="margin-bottom: var(--space-2xl);">
                                <label for="reviewer_name" style="display: block; font-weight: 600; color: var(--color-neutral-800); margin-bottom: var(--space-lg);">Tên của bạn:</label>
                                <input type="text" id="reviewer_name" name="reviewer_name" placeholder="Nhập tên của bạn..." 
                                       style="width: 100%; padding: var(--space-lg) var(--space-xl); border: 1px solid rgba(255,255,255,0.3); border-radius: var(--radius-lg); background: var(--gradient-glass); backdrop-filter: blur(10px); font-size: var(--text-base); color: var(--color-neutral-800);"
                                       required>
                            </div>
                            
                            <!-- Comment Text Area -->
                            <div style="margin-bottom: var(--space-2xl);">
                                <label for="comment" style="display: block; font-weight: 600; color: var(--color-neutral-800); margin-bottom: var(--space-lg);">Bình luận của bạn:</label>
                                <textarea id="comment" name="comment" rows="4" placeholder="Chia sẻ trải nghiệm của bạn về sản phẩm này..."
                                          style="width: 100%; padding: var(--space-lg) var(--space-xl); border: 1px solid rgba(255,255,255,0.3); border-radius: var(--radius-lg); background: var(--gradient-glass); backdrop-filter: blur(10px); font-size: var(--text-base); color: var(--color-neutral-800); resize: vertical; min-height: 120px; font-family: inherit;"
                                          required></textarea>
                            </div>
                            ...
                            <div style="text-align: center;">
                                <button type="submit" name="submit_review" 
                                        style="background: var(--gradient-primary); color: var(--color-neutral-0); border: none; padding: var(--space-lg) var(--space-3xl); border-radius: var(--radius-xl); font-size: var(--text-base); font-weight: 600; cursor: pointer; transition: all var(--transition-fast); box-shadow: var(--shadow-lg);"
    ...
    <script>
        ...
        function resetPreview() {
            if (currentRating > 0) {
                ...
                document.getElementById('rating-text').textContent = ratingTexts[currentRating];
            } else {
                updateStarDisplay(0);
